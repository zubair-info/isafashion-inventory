    ...
    <section class="wrapper-invoice">
      ...
      <div class="invoice">
        <div class="invoice-information">
          <p class="text-uppercase" style="font-size: 22px;"> <b >Chalan Id</b> : #{{ App\Models\KnittingReceivedLekraBrand::find($knitting_received_lekrabrand_id)->send_chalan_id }}</p>
          <p><b>Date </b>: {{ App\Models\KnittingReceivedLekraBrand::find($knitting_received_lekrabrand_id)->date}}</p>
        ...
        <div class="invoice-body">
            <table class="table" style="width:100%;margin-top:20px;">
                ...
                {{-- {{dd($all_suta_brand)}} --}}
                  ...
                  @foreach (App\Models\KnittingReceivedLekraBrand::find($knitting_received_lekrabrand_id)->get() as $key=>$knitting_received_lekrabrand)   
                    <tbody>
                        <tr>
                            <td>{{$key+1}}</td>
                            <td>{{$knitting_received_lekrabrand->rel_to_company->company_name}}</td>
                            <td>{{$knitting_received_lekrabrand->date}}</td>
                            <td>{{$knitting_received_lekrabrand->rel_to_lekra_brand->lekra_brand_name}}</td>
                            <td>{{$knitting_received_lekrabrand->lekra_cartoon}}</td>
                            <td>{{$knitting_received_lekrabrand->lekra_rate}}</td>
                        </tr>                                   
                    </tbody>
                 @endforeach
