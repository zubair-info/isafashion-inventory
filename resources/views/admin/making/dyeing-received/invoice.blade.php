    ...
    <section class="wrapper-invoice">
      ...
      <div class="invoice">
        <div class="invoice-information">
            ...
            <p class="text-uppercase" style="font-size: 22px;"> <b >Chalan Id</b> : #{{ App\Models\MakingDyeingRecived::find($dyeing_multiple_id)->send_chalan_id}}</p>
          <p><b>Date </b>: {{ App\Models\MakingDyeingRecived::find($dyeing_multiple_id)->created_at->format('d-M-Y')}}</p>
        </div>
        <!-- logo brand invoice -->
        ...
        <div class="invoice-body">
          <table class="table" style="width:100%">
            ...
            <tbody>
                ...
                @foreach (App\Models\MakingDyeingMultipleReceived::where('dyeing_send_id', $dyeing_multiple_id)->get() as $key => $dyeing_send_multiple_id)
                <tr>
                    <td>{{ $key + 1 }}</td>
                    <td>{{$dyeing_send_multiple_id->received_chalan_id}}</td>
                    <td>{{$dyeing_send_multiple_id->color}}</td>
                    <td>{{$dyeing_send_multiple_id->style}}</td>
                    <td>{{$dyeing_send_multiple_id->lot_no}}</td>
                    <td>{{$dyeing_send_multiple_id->body}}</td>      
                    <td>{{$dyeing_send_multiple_id->rib}}</td>
                    <td>{{$dyeing_send_multiple_id->balance}}</td>
                    {{-- <td>{{$dyeing_send_multiple_id->total_used_lekra}}</td> --}}
