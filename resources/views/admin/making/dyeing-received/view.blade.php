                ...
                <div class="card-body">
                    <div class="table-responsive">
                        ...
                        <table class="table" style="width:100%">
                            ...
                            {{-- {{dd($all_dyeing_received)}} --}}
                
                            @foreach ($all_dyeing_received as $key=>$dyeing_received)   
                                <tbody>
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$dyeing_received->received_chalan_id}}</td>
                                        {{-- <td>{{$dyeing_received->dyeing_send_id}}</td> --}}
                                        <td>{{$dyeing_received->date}}</td>
                                        <td>{{$dyeing_received->color}}</td>
                                        <td>{{$dyeing_received->style}}</td>
                                        <td>{{$dyeing_received->lot_no}}</td>
                                        <td>{{$dyeing_received->body}}</td>
                                        <td>{{$dyeing_received->body}}</td>
                                        <td>{{$dyeing_received->rib}}</td>
                                        <td>{{$dyeing_received->balance}}</td>
                                        <td>
                                            <a  class="btn btn-sm bg-warning-light"href="{{route('dyeingReceivedMultipleEdit',$dyeing_received->id)}}">
