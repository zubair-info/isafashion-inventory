    ...
    <section class="wrapper-invoice">
      ...
      <div class="invoice">
        <div class="invoice-information">
            ...
            <p class="text-uppercase" style="font-size: 22px;"> <b >Chalan Id</b> : #{{ App\Models\MarkatReceived::find($markat_received_id)->received_chalan_id}}</p>
            <p><b>Date </b>: {{ App\Models\MarkatReceived::find($markat_received_id)->date}}</p>
        ...
        <div class="invoice-body">
          <table class="table" style="width:100%">
            ...
            <tbody>
                ...
                @foreach (App\Models\MarkatMultipleReceived::where('received_markat_id', $markat_received_id)->get() as $key => $markat_received_id)
                <tr>
                    <td>{{ $key + 1 }}</td>
                    <td>{{$markat_received_id->received_markat_id}}</td>
                    <td>{{$markat_received_id->rel_to_kapor->kapor_name}}</td>
                    <td>{{$markat_received_id->roll}}</td>      
                    <td>{{$markat_received_id->rate}}</td>      
                    <td>{{$markat_received_id->body}}</td>
                    <td>{{$markat_received_id->balance}}</td>
                    {{-- <td>{{$markat_received_id->total_used_lekra}}</td> --}}
