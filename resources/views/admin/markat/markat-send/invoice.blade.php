    ...
    <section class="wrapper-invoice">
      ...
      <div class="invoice">
        <div class="invoice-information">
            ...
            <p class="text-uppercase" style="font-size: 22px;"> <b >R.Chalan No</b> : #{{ App\Models\MarkatSend::find($markat_send_id)->received_chalan_id}}</p>
            <p><b>Date </b>: {{ App\Models\MarkatSend::find($markat_send_id)->created_at->format('d-M-Y')}}</p>

        </div>
        <!-- logo brand invoice -->
        <div class="invoice-logo-brand">
          <!-- <h2>Tampsh.</h2> -->
          {{-- <img style="background: #6c757d; padding:10px;" src="{{ asset('backend/assets/img/bitBirdslogo.png')}}" alt="" /> --}}
        ...
        <div class="invoice-body">
          <table class="table" style="width:100%">
            ...
            @foreach (App\Models\MarkatMultipleSend::where('markat_send_id', $markat_send_id)->get() as $key => $markat_send_id)
                <tr>
                    <td>{{ $key + 1 }}</td>
                    <td>{{$markat_send_id->send_chalan_id}}</td>
                    <td>{{$markat_send_id->date}}</td>      
                    <td>{{$markat_send_id->style}}</td>      
                    <td>{{$markat_send_id->lot_no}}</td>
                    <td>{{$markat_send_id->body}}</td>
                    <td>{{$markat_send_id->balance}}</td>
